...
import { updatSelectedLanguge, getDebtors, SelectedPaymentPopup, filteredDebtorsList } from "../../../Actions/appActions"
...
const ManageDebtors = () => { 
  ...
  const currentPosts = dataFromStore.appreducers.debtorsListFiltered.slice(indexOfFirstPost, indexOfLastPost)
  ...
  const filterList = (e) => {
    var updateList = dataFromStore.appreducers.debtorsList;
    ...
    dispatch(filteredDebtorsList(updateList))
  }
  ...
  return (
    <div className="row" >
                      ...
                      <tr key={id}>
                        ...
                        <td>
                          <button type="button" className="btn btn-outline btn-sm mb-1" style={{ backgroundImage: 'linear-gradient(160deg, rgb(103, 183, 225) 25%, rgb(15, 105, 153))', border: 'none', color: "white" }} onClick={() => dispatch(SelectedPaymentPopup(res.ssn))}>
                            ...
                            <PaymentPlans showId={res.ssn} paymentDetails={res.paymentPlan} />
                          </button>
                        </td>
      ...
      <div style={{ marginLeft: 600, marginBottom: 10 }}>
        <Pagination postsPerPage={postsPerPage} totalPosts={dataFromStore.appreducers.debtorsListFiltered.length} paginate={paginate} />
      </div>
